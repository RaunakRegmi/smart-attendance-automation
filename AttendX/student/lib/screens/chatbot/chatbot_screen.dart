...
import 'context_window_sheet.dart';
...
class _ChatbotScreenState extends State<ChatbotScreen> {
  ...
  Future<void> _initSession() async {
    // Restore the durable, per-account conversation from the server. The local
    // SharedPreferences copy is only an offline fallback now.
    try {
      final serverMsgs = await ChatService.fetchConversation();
      if (!mounted) return;
      setState(() {
        _messages
          ..clear()
      ...
      // Offline / error → fall back to the cached transcript below.
    }
  ...
  Future<void> _sendMessage(String text) async {
    ...
    // Streams the reply token-by-token; the typing bubble is replaced by a live
    // assistant bubble that grows as tokens arrive.
    var started = false;
    var acc = '';
    try {
      await for (final ev in ChatService.streamMessage(trimmed)) {
        if (!mounted) return;
        if (ev.type == 'token') {
          acc += ev.delta ?? '';
          setState(() {
            ...
            if (!started) {
              _messages.add(ChatMessage.fromAssistant(acc));
              started = true;
            } else {
              _messages[_messages.length - 1] = ChatMessage.fromAssistant(acc);
            }
          });
          _scrollToBottom();
        } else if (ev.type == 'error') {
          acc = started ? '$acc\n[${ev.message ?? 'error'}]' : (ev.message ?? 'Connection error. Please try again.');
          setState(() {
            _messages.removeWhere((m) => m.isTyping);
            if (!started) {
              _messages.add(ChatMessage.fromAssistant(acc));
      ...
      if (!started && mounted) {
        setState(() {
          _messages.removeWhere((m) => m.isTyping);
          _messages.add(ChatMessage.fromAssistant("I didn't understand. Please rephrase."));
        });
      }
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _openContextSheet() async {
    try {
      final info = await ChatService.fetchContext();
      if (!mounted) return;
      await showContextWindowSheet(context, info);
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Could not load context info')),
      );
    }
  }
  ...
  Future<void> _confirmClearChat() async {
    ...
    if (confirmed == true && mounted) {
      ...
      try {
        ...
        // Non-fatal: local cache is still cleared below.
      }
      final prefs = await SharedPreferences.getInstance();
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          // Context window ("what I remember")
          IconButton(
